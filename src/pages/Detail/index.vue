        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前咱们的路由跳转，从A路由跳转到B路由，现在这里在加入购物车进行路由跳转之前，需要发请求把你购买的产品信息通过请求的形式通知服务器，服务器进行相应的存储 -->
...
export default {
  ...
  computed: {
    //mapGetters里面的写法：传的的是数组，因为getters计算没有划分模块【home,search】
    ...mapGetters(["categoryView", "skuInfo", "spuSaleAttrList"]),
    skuImageList() {
      //如果服务器的数据没有回来，skuInfo是个空对象，空对象.属性是undefined，至少应该是一个空数组
      return this.skuInfo.skuImageList || [];
    },
  },
  methods: {
    //产品的售卖属性值切换高亮
    changActive(saleAttrValue, attr) {
      // console.log(a);
      //遍历全部售卖属性值中的isChecked，使其为零即没有高亮了
      attr.forEach((item) => {
        item.isChecked = 0;
      });
      //电机的那个售卖属性值高亮
    ...
    //async与await是成对出现的
    async addShopCart() {
      //1：在点击接入购物车这个按钮的时候，做的第一件事情，将参数带给服务器（发请求），筒子服务器加入购物车的产品是谁
      //2：你需要知道这次请求成功还是失败，如果成功就进行路由跳转；如果失败就需要给用户提示
      try {
        //如果成功了。。。。
        //await等待promise的成功
        ...
        //3：成功，就进行路由的跳转
        //4：在路哟一天赚的时候还需要将产品的信息带给下一级路由组件（一些简单的数据如skuNum，通过query形式给路由组件传递过去；一些比较复杂的数据如skuInfo，通过会话存储（不持久化，会话结束就消失）），
        //注意：本地存储|会话存储，一般存储的是字符串
